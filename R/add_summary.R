...
add_summary <- function(p, fun = "mean_se", error.plot = "pointrange",
  ...
  line_geoms <- c(
    "errorbar", "linerange", "upper_errorbar", "lower_errorbar",
    "upper_linerange", "lower_linerange", "crossbar"
  )

  allowed.error.plot <- c(
    "pointrange", "linerange", "crossbar", "errorbar",
...
.format_error <- function(d, error.limit = "both") {
  if (error.limit == "upper") {
    d$ymin <- d$y
  } else if (error.limit == "lower") {
    d$ymax <- d$y
