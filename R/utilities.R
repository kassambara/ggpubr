...
.set_ticks <-
  function(ticks = TRUE, tickslab = TRUE, font.tickslab = NULL,
    ...
    if (is.null(font.xtickslab)) {
      ...
      font.x <- .parse_font(font.xtickslab)
    }
...
.set_legend <- function(p, legend = NULL,
  ...
  if (!.is_empty(legend.title) && !(is.character(legend.title) && legend.title == "")) {
    if (.is_list(legend.title)) {
      p <- p + do.call(ggplot2::labs, legend.title)
    } else {
...
.summary_functions <- function() {
  c(
    "mean", "mean_se", "mean_se_", "mean_sd", "mean_ci",
...
.ggannotate <- function(label, size = 12, coord = c(NULL, NULL)) {
  if (is.null(unique(coord))) {
    grob <- grid::grobTree(grid::textGrob(label,
...
.get_not_numeric_vars <- function(data_frame) {
  ...
  if (sum(!is_numeric) == 0) {
    ...
    res <- colnames(data_frame[, !is_numeric, drop = FALSE])
  }
  res
}
...
.plotter <- function(fun, data, x, y, combine = FALSE, merge = FALSE,
  ...
  # when multiple plots
  opts <- opts %>%
    c(list(title = title, xlab = xlab, ylab = ylab)) %>%
    .compact()
  data <- opts$data
...
.geom <- function(p, .layer = 1) {
  . <- NULL
  if (is.null(p) | .is_empty(p$layers)) {
    return("")
  }
